...
//
// exports.crearAvisos = onSchedule("0 6 * * *", async () => {
//     const tramitesQuery = firestore.collection("tramites").where("estado", "in", ["ACTIVO", "Entregado"]);
//     const tramitesSnapshot = await tramitesQuery.get();
//
...
//
//         if (mensaje) {
//             const avisoRef = firestore.collection("avisos").doc();
//             batch.set(avisoRef, {
//                 fechaFinTrammite: doc.data().fechaFinTramite,
//                 diasRestantes: diasRestantes,
//                 mensaje: mensaje,
...
//
// exports.eliminarAvisosEntregados = onSchedule("0 5 * * *", async () => {
//     const tramitesQuery = firestore.collection("tramites").where("estado", "==", "Entregado");
//     const tramitesSnapshot = await tramitesQuery.get();
//
//     const tramites = tramitesSnapshot.docs.map(doc => doc.data().codigo);
//
//     const avisosQuery = firestore.collection("avisos").where("codigo", "in", tramites);
//     const avisosSnapshot = await avisosQuery.get();
//
//     const batch = firestore.batch();
//
...
//         const tramiteCodigo = newData.codigo;
//
//         const avisosQuery = firestore.collection("avisos").where("codigo", "==", tramiteCodigo);
//         const avisosSnapshot = await avisosQuery.get();
//
//         const batch = firestore.batch();
//
...
// });
//
// exports.crearNotificacionLogin = auth.user().beforeSignIn((user, context) => {
//     logger.log("Creando notificacion de login");
//     const aviso = new Aviso(user, new Date(), context.ipAddress);
//     return firestore.collection("login").add(aviso)
//         .then(() => logger.log("Notificacion de login creada"))
//         .catch((error) => logger.error("Error al crear la notificacion de login", error));
// });
